...
│     ├─ RF.py：以 ReliefF 評估各植生指數的重要性
│     ├─ K_sensitivity.py：比較 ReliefF 在不同鄰居數 K 下的重要性結果
│     ├─ K_mean.py：整合選定 K 值的 ReliefF 結果，取得較穩定的重要植生指數
│     ├─ ReliefF_source_code.py：ReliefF 計算的原始或輔助程式
│     │
│     └─ KS_v2
│        ├─ divided_high_low_vis_v2.py：將完整植生指數表分為高濃度與低濃度資料
│        ├─ KS_high_v2.py：對高濃度樣本執行 Kennard–Stone 資料切分
│        ├─ KS_low_v2.py：對低濃度樣本執行 Kennard–Stone 資料切分
│        └─ KS_npy_v2.py：對全部濃度或平均光譜資料執行 Kennard–Stone 資料切分
│
