# v3 繼承 v2
# 多做了feature selection，並改成一次跑完三個資料集
# 結果存放在ml_result/4_feature_selection


# -*- coding: utf-8 -*-
"""
Loop 3 concentrations (high/low/npy) -> load KS split -> select given bands -> CV on train -> test eval
Outputs:
  ML_result/4_feature_select/<conc>/ml_ks_cv_<conc>_v3.csv
  ML_result/4_feature_select/<conc>/confmat_<Model>_<conc>.csv
  ML_result/4_feature_select/all_concentrations_results.csv
"""
import warnings
...
# ------------------ 共用設定 ------------------
BASE_ROOT = Path(r"D:/Users/Amanda/PycharmProjects/test/test_0322_new.raw_pca(wait)")
CONCS = ["high", "low", "npy"]
RANDOM_STATE = 42
np.random.seed(RANDOM_STATE)

SELECTED_WLS = np.array([
    # LDA
...
def load_raw_spectra_by_list(df: pd.DataFrame, base_dir: Path, label_encoder: LabelEncoder):
    ...
    X = np.vstack(specs) if specs else np.empty((0,0))
    y = label_encoder.transform(labels) if labels else np.array([])
    return X, y, names

def select_bands_by_wavelength(X: np.ndarray, wavelengths: np.ndarray,
                               wanted: np.ndarray, tol: float):
    """回傳 X[:, mask] 與實際被選到的波長"""
    mask = np.zeros_like(wavelengths, dtype=bool)
    for w in wanted:
        mask |= np.isclose(wavelengths, w, atol=tol)
    return X[:, mask], wavelengths[mask], mask

def eval_with_cv_and_test(models: dict, X_train, y_train, X_test, y_test, out_dir: Path, tag: str):
    ...
    for name, pipe in models.items():
        ...
        print(f"Test Spec     : {spec:.4f}" if not np.isnan(spec) else "Test Spec     : N/A")
        print(f"Test Sens     : {sens:.4f}" if not np.isnan(sens) else "Test Sens     : N/A")
        print(f"Test MCC      : {mcc:.4f}" if not np.isnan(mcc) else "Test MCC      : N/A")

        cm_path = out_dir / f"confmat_{name.replace(' ','_').replace('(','').replace(')','')}_{tag}.csv"
...
# main
def run_one_concentration(conc: str) -> pd.DataFrame:
    print(f"\n================= Running concentration: {conc} =================")
    base_dir = BASE_ROOT / conc
    assert (base_dir / "healthy").exists() and (base_dir / "unhealthy").exists(), f"找不到 {base_dir}/healthy 或 /unhealthy"

    # ks資料集區分
    # =====================================================================
    split_dir = Path(f"ks_{conc}_divided_result")
    train_list = load_list_csv(split_dir / f"ks_split_train_{conc}.csv")
    test_list  = load_list_csv(split_dir / f"ks_split_test_{conc}.csv")
    # =====================================================================

    # label encoder
    le = LabelEncoder().fit(pd.concat([train_list["Label"], test_list["Label"]], axis=0).astype(str))

    # load spectra by list
    X_train, y_train, _ = load_raw_spectra_by_list(train_list, base_dir, le)
    X_test,  y_test,  _ = load_raw_spectra_by_list(test_list,  base_dir, le)
    if X_train.size == 0 or X_test.size == 0:
        raise ValueError(f"[{conc}] 訓練或測試資料為空，請檢查清單與檔案路徑。")
    print(f"[{conc}] Train: X={X_train.shape}, Test: X={X_test.shape}")

    # wavelengths
    wl_all = np.load("wavelengths.npy").astype(float)
    assert X_train.shape[1] == wl_all.shape[0], f"[{conc}] 光譜維度與波長數量不符"

    # select wanted bands
    X_train_sel, wl_sel, mask = select_bands_by_wavelength(X_train, wl_all, SELECTED_WLS, TOL)
    X_test_sel  = X_test[:, mask]
    print(f"[{conc}] 選取波段 {len(wl_sel)} 個（±{TOL} nm），範例前 8 個：", np.round(wl_sel[:8], 2))
    print(f"[{conc}] 篩選後: Train={X_train_sel.shape}, Test={X_test_sel.shape}")

    # small sanity
    assert X_train_sel.shape[1] > 0 and X_test_sel.shape[1] > 0, f"[{conc}] 無選到任何波段，請調整 TOL 或波長清單。"

    # eval
    models = build_models()
    out_dir = Path("ML_result/4_feature_select") / conc
    df_res = eval_with_cv_and_test(models, X_train_sel, y_train, X_test_sel, y_test, out_dir, tag=conc)

    # per-concentration csv
    per_csv = out_dir / f"ml_ks_cv_{conc}_v3.csv"
    df_res.to_csv(per_csv, index=False)
    print(f"[{conc}] 已寫出：{per_csv}")
    return df_res

def main():
    all_rows = []
    for conc in CONCS:
        df = run_one_concentration(conc)
        all_rows.append(df)
    all_df = pd.concat(all_rows, ignore_index=True)
    out_all = Path("ML_result/4_feature_select") / "all_concentrations_results.csv"
