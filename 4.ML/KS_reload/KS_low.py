...
# ====================== 使用者設定 ======================
CONCENTRATION = "low"  # 對應 {CONCENTRATION}/healthy, {CONCENTRATION}/unhealthy
VI_CSV_PATH = Path(r"D:/Users/Amanda/PycharmProjects/test/test_0322_new.raw_pca(wait)/vi_40_low.csv")
BASE_DIR = Path(fr"D:/Users/Amanda/PycharmProjects/test/test_0322_new.raw_pca(wait)/{CONCENTRATION}")

# 用來做 KS 分割的 VI 欄位（可自由調整）
# 80% up -> 8個
VI_FEATURES = ["MTCI", "VOG3", "VOG1", "Chlorophyll Index RedEdge", "CInir","TCARI/OSAVI","CIrededge","NDRE"]
...
def kennard_stone(X: np.ndarray, n_train: int) -> np.ndarray:
    ...
    if n_train >= n_samples:
        n_train = n_samples - 1  # 至少留 1 筆給測試
