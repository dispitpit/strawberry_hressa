...
class HSI3DDataset(Dataset):
    """
    whole-image 3D HSI classification dataset

    2D 版輸出: (C, H, W)
    3D 版輸出: (1, D, H, W)

    其中：
    - C = 32 個 selected bands（在 2D CNN 中視為 input channels）
    - D = 32 個 selected bands（在 3D CNN 中視為 spectral depth）

    支援：
    1. real 300-band -> selected bands
    2. fake 32-band  -> 直接用
    3. normalization
    4. 只 resize 空間尺寸到固定 TARGET_HW；保留光譜深度 D
    ...
    def _make_cache_key(self, src_path: str) -> str:
        ...
        raw = f"3d|{src_path}|hw={hw_tag}|bands={','.join(map(str, self.selected_bands))}"
    ...
    def _load_cube_raw(self, path: str):
        ...
        if self.target_hw is not None:
            ...
            x = F.interpolate(
                x.unsqueeze(0),
                size=(d, self.target_hw[0], self.target_hw[1]),
                mode="trilinear",
                align_corners=False,
            ).squeeze(0)
