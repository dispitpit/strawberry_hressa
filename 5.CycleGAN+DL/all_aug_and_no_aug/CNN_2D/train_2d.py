# train_2d.py
# 這版是完整整合版：
# 1. 一次跑 concentration = low / high
# 2. 一次跑 batch size = [8, 16, 32, 64]
# 3. 每個組合都跑 no_aug / aug
# 4. 每個組合都輸出：
...
from dataset_hsi_2d_patch import HSI2DDataset
...
def build_master_result_root() -> Path:
    out_dir = cfg.RESULTS_ROOT / f"grid_compare_{_now_tag()}"
...
def train_one_epoch(model, loader, optimizer, criterion, device, scaler, amp_enabled=False, debug_first_batch=False):
    ...
    for batch_idx, (x, y) in enumerate(loader):
        ...
        if debug_first_batch and batch_idx == 0:
            print(f"[DEBUG] first train batch x.shape={tuple(x.shape)}, x.device={x.device}, y.device={y.device}")
...
def evaluate(model, loader, criterion, device, amp_enabled=False, debug_first_batch=False):
    ...
    for batch_idx, (x, y) in enumerate(loader):
        ...
        if debug_first_batch and batch_idx == 0:
            print(f"[DEBUG] first val/test batch x.shape={tuple(x.shape)}, x.device={x.device}, y.device={y.device}")
...
def prebuild_cache(records_df: pd.DataFrame, cache_dir: Path, desc: str):
    ...
    total = len(paths)
    print(f"[INFO] Prebuild cache for {desc}: {total} samples")
...
def train_final_and_test(exp_name, train_real_df, test_real_df, aug_df, use_aug,
    ...
    print(f"\n[Final Train -> Test] concentration={concentration} exp={exp_name} batch_size={batch_size}")
    ...
    model = make_model()
    print("[INFO] final model device:", next(model.parameters()).device)
    ...
    for epoch in range(1, chosen_epochs + 1):
        ...
        print(f"[Final] Epoch {epoch:03d} | train loss {tr_loss:.4f} acc {tr_acc:.4f}")
...
def run_experiment(use_aug: bool, train_real_df: pd.DataFrame, test_real_df: pd.DataFrame,
    ...
    print("=" * 100)
    print(f"Experiment: concentration={concentration} | exp={exp_name} | batch_size={batch_size}")
    print("=" * 100)
...
def write_compare_summary(result_root: Path, concentration: str, batch_size: int, results: dict):
    ...
    print("\n[INFO] compare_summary saved to:", result_root / "compare_summary.csv")
    print(final_df)
    return final_df
...
def run_one_batch_size(train_real_df: pd.DataFrame, test_real_df: pd.DataFrame, aug_df: pd.DataFrame,
    ...
    print("\n" + "#" * 100)
    print(f"[Grid] concentration={concentration} | batch_size={batch_size}")
    print("#" * 100)
...
def main():
    ...
    print("\n" + "=" * 100)
    print("[DONE] grid_compare_summary.csv")
    print(master_root / "grid_compare_summary.csv")
    print(master_df)
    print("\n[DONE] grid_compare_summary_compact.csv")
