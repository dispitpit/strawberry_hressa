...
import inspect
print(inspect.getsource(ReliefF))
'''
偽程式碼
fit(X, y):
  set _X, _y, n, d
  if n_neighbors is float: n_neighbors = int(n_neighbors * n * 0.5)
  判斷 _class_type (binary/multiclass/continuous) 與 mcmap
  數欄名 headers = X01..Xd；計缺值數
  attr = 各欄離散/連續與 min/max/range/std
  data_type = discrete / continuous / mixed
  (attrdiff, cidx, didx) = _dtype_array()
  (xc, xd) = (X[:, cidx], X[:, didx])
  if 有缺值: _distance_array = _distarray_missing(...)
  else:      _distance_array = _distarray_no_missing(...)
  對每個樣本 i：NN[i] = _find_neighbors(i)
  feature_importances_ = Σ ReliefF_compute_scores(i, NN[i], ...)
  del _distance_array
  top_features_ = argsort(feature_importances_)[::-1]
  return self
'''

'''
特徵選擇器: 給定X(n_samples, n_features)、y(n_samples)
n_features_to_seledt: 最後要保留的特徵數
n_neighbors: 每個樣本找k個同類(hit)+k個異類(miss)來更新分數
discrete_threshold: 判斷離散/連續的門檻
n_jobs: 平行化計算分數用的核心數
def fit(X, y): 判斷特徵為離散還是連續，連續的話->cityblock，離散-> Hamming距離
_find_neighbors找鄰近
_run_algorithm計分
'''
# 套件來自參考文獻：
# Robnik-Šikonja, Marko & Kononenko, Igor (1997). “Overcoming the Myopia of Inductive Learning Algorithms with ReliefF.” Applied Intelligence, 7(1), 39–55.
# 發表人（作者）是 Marko Robnik-Šikonja 和 Igor Kononenko。

# D:\Users\Amanda\PycharmProjects\test\.venv1\Scripts\python.exe
# ("C:/Program Files/JetBrains/PyCharm 2025.2.1.1/plugins/python-ce/helpers/pydev/pydevconsole.py"
